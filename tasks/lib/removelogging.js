exports.init = function(grunt) {
  ...
  return function(src, opts) {
    ...
    if(!("namespace" in opts)) {
        opts.namespace = ["console", "window.console" , "chrome.extension.getBackgroundPage\\(\\).console"];
    }
    ...
    rConsole = new RegExp("(" + opts.namespace.join("|") + ")" + ".(?:" + opts.methods.join("|") + ")\\s{0,}\\([^\r\n]*\\)(?!\\s*[;,]?\\s*\\/\\*\\s*RemoveLogging:skip\\s*\\*\\/)\\s{0,}?", "gi");
